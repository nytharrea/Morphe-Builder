...
| File | Purpose |
|---|---|
| `apkmirror.py` | Thin orchestration layer: resolves an app + version to the right APKMirror URL from the `org`/`slug`/`release_slug` its `catalog/apps.yaml` entry supplies, fetches each page through `flaresolverr.py`, works through APKMirror's variant-row → download-button → confirm-page → final-file hop by reading each page's link `href` directly rather than clicking anything, and streams the file to disk - retrying through Cloudflare challenges via `apkmirror_challenge.py` and reading each page's HTML via `apkmirror_parse.py` as it goes. Its own public API (`close_session`, `download_apk`, `get_latest_listing`) is everything outside this module needs; internally it also re-exports several of `apkmirror_parse.py`'s functions under their old private names, purely so `tests/test_apkmirror.py` didn't need to change when this file was split out of what used to be one ~520-line module. |
| `apkmirror_challenge.py` | Cloudflare challenge-page detection (`looks_like_challenge`) and the escalating cooldown built on it (`register_challenge`, `apply_global_cooldown`, the `ChallengeCooldownWait`/`BudgetExceeded` retry strategies) - the `ChallengePresent` exception these raise is what `apkmirror.py`'s retry loops key off of. Deliberately has no dependency on HTML parsing: `looks_like_challenge` takes a page-text-extraction function as a parameter instead of importing one from `apkmirror_parse.py`, so this module only ever knows "challenge detected or not," never how a page got turned into text. |
| `apkmirror_parse.py` | Everything about turning an already-fetched page's HTML into something useful: picks the right variant row by architecture/DPI priority (with a bundle-only special case for apps like Instagram that APKMirror only ships as a split `.apkm`), detects 404 pages, and pulls version numbers and listing links out of a page. No network calls and no retry logic live here. |
...
| `github_app.py` | Downloads apps that are mirrored as a direct GitHub release asset instead, per an app's `apk_source` in `catalog/apps.yaml`. Plain HTTP via `morphe_builder/http.py`, no browser or FlareSolverr involved. Its `download_apk()` accepts the same `force_build` parameter `apkmirror.py`'s does (so `scripts/patch.py` can call either fetcher with one shared call signature) but never actually uses it - a GitHub release has no equivalent of an APKMirror variant row to filter by. |
...
| `tests/` | `pytest` unit tests for `morphe_builder/retry.py`, `morphe_builder/validate.py`, `morphe_builder/catalog.py`'s app/build merge logic, `morphe_builder/notify.py`, `morphe_builder/apk/versions.py`, `morphe_builder/apk/patcher.py` (including that secrets never leak into its logged command line), `morphe_builder/apk/verify.py`'s pending/known signature flow, `morphe_builder/fetchers/apkmirror.py` + `apkmirror_parse.py`'s HTML-parsing/selection logic, `morphe_builder/fetchers/flaresolverr.py`'s download validation, `morphe_builder/fetchers/github_app.py`'s tag/asset selection, `morphe_builder/fetchers/release_assets.py`'s release-selection logic, `scripts/prepare_release.py`'s output shape, and `scripts/finalize_release.py`'s asset-matching logic. |
| `.gitignore` | Excludes `__pycache__`, virtualenvs, downloaded APKs, decoded `.keystore` files, `.env` files, `diagnostics/`, `artifacts/`, `dist/`, and the `ruff`/`mypy`/`pytest` cache directories. |
